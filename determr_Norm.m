%%%%%下面是determr_Norm函数的定义。
%%%%%%3A. 以下反求三维空间中的裂隙总数，并确定三维空间中的r服从何种分布，这是r2服从正态分布的情况
function [Normexpt, Normvarp, Normchi2, Gamexpt, Gamvarp, Gamchi2, Lognexpt, Lognvarp, Lognchi2]=determr_Norm(aa,zz,theta,nm2,Normnv2)
uty=unifrnd(0,1,1,100000);	%生成一组u/r的随机数
chtor2=1-uty.^2; 		%相应于u/r，生成一组ch2/r2的随机数
nm=nm2;	%正态分布用此二式计算nm,nv
nv=sqrt(Normnv2);
r2=normrnd(nm,nv,1,100000);
Nr2=r2;	%若为正态分布，则需执行这部分代码，因正态分布可能出现负值
j=1;
for i=1:100000
if r2(i)>0
r2(j)=Nr2(i);
